...
public:
    ...
    // 状态标记
    bool takeoff_done{false}; // 标记是否完成起飞

    // --- 仿照 se3Ctrl 的状态枚举 ---
    enum State_t
    {
        MANUAL_CTRL,   // 对应 WAITING_FOR_OFFBOARD (等待切入OFFBOARD)
        AUTO_RUNNING,  // 对应 MISSION_EXECUTION (任务执行：起飞、悬停、轨迹)
        AUTO_LAND      // 对应 LANDING (可选)
    };
